...
本实验支持以下两种平台，Makefile 会根据 `uname -m` 自动检测并切换编译器与 QEMU 参数：

| 项目 | Ubuntu AMD64 (x86_64) | 银河麒麟 ARM64 (aarch64) |
|---|---|---|
| C++ 编译器 | `g++ -m32` | `i686-elf-g++`（交叉编译） |
| 链接器 | `ld` | `i686-elf-ld` |
| QEMU | `qemu-system-i386 -hda` | `qemu-system-x86_64 -drive file=...,format=raw` |
| GDB | `gdb` | `gdb-multiarch` |
| 运行方式 | `make run` | `make run` 或 `make monitor`（无图形界面时） |

> **ARM64 说明**：在银河麒麟等无图形界面的 ARM64 环境下，可以使用 `make monitor` 启动 QEMU 并开启 telnet 监控端口，通过 QEMU Monitor 的 `xp` 命令读取 VGA 显存来验证输出。
...
> **ARM64 环境**：在银河麒麟 ARM64 环境下，使用 `make build && make monitor` 启动，然后通过 QEMU Monitor 读取 VGA 显存验证输出。
...
我们称“访问共用资源的代码”为**临界区**。自旋锁的基本思想是定义一个共享变量`bolt`，`bolt`会被初始化为0。在线程进入临界区之前，即访问共享变量之前，都需要去检查`bolt`是否为0。如果`bolt`为0，那么这个线程就会将`bolt`设置为1，然后进入临界区。待线程离开临界区后，线程会将`bolt`设置为0。如果线程在检查`bolt`时，发现`bolt`为1，说明有其他线程在临界区中。此时，这个线程就会一直在循环检查`bolt`的值（类似陀螺在原地旋转，所以被称为自旋），直到`bolt`为0，然后进入临界区。
...
```shell
make build && make run    # Ubuntu AMD64
make build && make monitor # 银河麒麟 ARM64（无图形界面）
```
...
在银河麒麟 ARM64 等无图形界面的环境下，可以使用 `make monitor` 启动 QEMU 并开启 telnet 监控端口。通过 QEMU Monitor 的 `xp` 命令可以直接读取 VGA 显存（`0xb8000`）来验证输出。

```shell
# 在 build 目录下启动
make monitor

# 在另一个终端连接 QEMU Monitor
telnet 127.0.0.1 45461   # src/1 的端口
telnet 127.0.0.1 45462   # src/2 的端口
telnet 127.0.0.1 45463   # src/3 的端口

# 在 QEMU Monitor 中读取 VGA 显存
xp /4000bx 0xb8000
...
VGA 显存格式：每个字符占 2 字节，偶数字节为 ASCII 字符，奇数字节为颜色属性。每行 80 个字符，共 25 行。
