...
> **多操作系统支持：** 本实验支持在 **Ubuntu** 和 **麒麟操作系统（Kylin OS）** 上完成。两者的主要差异在于包管理器不同（Ubuntu 使用 `apt`，麒麟OS 使用 `yum`）以及部分工具名称有所区别。
>
> **多架构支持：** 本实验以 x86（IA-32/x86_64）为目标架构。
> - **x86_64 环境**（Ubuntu 或麒麟OS）：所有实验内容均可直接完成。
> - **ARM64 环境**（麒麟OS ARM64 等）：NASM 可在 ARM64 上编译 x86 代码，QEMU 通过 TCG 模式模拟 x86 处理器，MBR 和 bootloader 均可正常编译运行。GDB 调试在 ARM64 上需要使用 QEMU Monitor 作为替代方案（详见 debug 章节）。
...
```shell
sudo yum makecache
sudo yum install nasm qemu-system-x86_64 gdb make
```

> **说明：**
> - 如果麒麟OS上没有 `qemu-system-i386`，可使用 `qemu-system-x86_64` 代替，效果完全一致。
> - ARM64 架构的麒麟OS上，`ld -melf_i386` 不可用，生成符号表时需使用实验一中构建的 `i686-elf-ld` 工具。
...
保护模式，是一种在80286系列之后，基于x86架构的CPU操作模式。在80286及以后，保护模式的引入使得**内存地址增加到32位**。因此，程序至少可以访问到$2^{32}=4G$的内存空间。保护模式与实模式相比，主要有以下差别。
...
CPU需要知道当前运行中**程序的段地址空间信息**，然后才能执行地址保护，阻止程序越界访问。段地址空间信息是通过**段描述符(segment descriptor)**来给出的。段描述符中包含了段基地址(段的起始地址)、段界限(段的长度)等，共计64字节，如下所示。上面一行是高32字节、下面一行是低32字节。
...
在保护模式下，所有**段描述符都会被集中放置**，这个集中放置的区域被称为**全局描述符表**(Global Descriptor Table，简称**GDT**)，GDT其实就是一个段描述符数组。
...
**保护模式下**的段寄存器依然是 16 位，但其中保存的不再是段地址，因为段地址保存在段描述符中。**段寄存器保存**的内容是**段选择子**(segment selector)。在段选择子是用来告诉CPU寻址时使用哪个段。所有的段都会被保存在全局描述符表(GDT)中，不难想到，段选择子是全局描述符表的索引，类似数组访问`array[i]`中的`i`，但段选择子中还会包含其他信息，如下所示。
...
```
  ...
  现在我们来到了**保护模式的真正开关——CR0**。 CR0 是 32 位的寄存器，包含了一系列用于控制处理器操作模式和运行状态的标志位，其**第0位是保护模式的开关位**，称为PE（protect mode enable）位。 PE置1，CPU 进入保护模式，代码如下。 
...
> **麒麟OS用户提示：** 若没有 `qemu-system-i386`，使用 `qemu-system-x86_64` 代替即可：
> ```shell
> qemu-system-x86_64 -hda hd.img -serial null -parallel stdio
> ```
...
我们以代码段选择子为例来解释**段选择子**的含义，段选择子的结构如下。
...
**x86_64 环境（Ubuntu 或 x86_64 麒麟OS）：**
...
ARM64 上的 `ld` 不支持 `-melf_i386` 参数，需使用实验一中构建的 i686-elf 交叉工具链：

```shell
nasm -g -f elf32 mbr.asm -o mbr.o
i686-elf-ld -o mbr.symbol -melf_i386 -N mbr.o -Ttext 0x7c00

nasm -g -f elf32 bootloader.asm -o bootloader.o
i686-elf-ld -o bootloader.symbol -melf_i386 -N bootloader.o -Ttext 0x7e00
```

> 注意：生成符号表时需要先删除源码中的 `org` 伪指令，因为 ELF 格式不支持 `org`。

## ARM64 上的调试替代方案：QEMU Monitor

ARM64 架构的麒麟OS上，系统自带的 GDB 仅支持 AArch64 架构，无法调试 x86 目标。此时可以使用 **QEMU Monitor** 作为替代调试手段。

QEMU Monitor 启动方式：

```shell
# 方式一：直接使用 monitor
qemu-system-x86_64 -drive file=hd.img,format=raw -display none -monitor stdio

# 方式二：通过 telnet 访问 monitor
qemu-system-x86_64 -drive file=hd.img,format=raw -monitor telnet:127.0.0.1:4444,server,nowait &
sleep 2
ncat 127.0.0.1 4444
```

在 QEMU Monitor 中常用的调试命令：

| 命令 | 功能 |
|------|------|
| `info registers` | 查看所有寄存器内容（包括段选择子、CR0、GDT等） |
| `x /Ni addr` | 反汇编从 addr 开始的 N 条指令 |
| `xp /Nx addr` | 查看物理地址 addr 处的 N 个数据 |
| `info mem` | 查看内存映射 |
| `stop` / `cont` | 暂停 / 继续执行 |
| `gdbserver` | 启动 GDB 远程调试端口 |

例如，查看保护模式下的寄存器状态：

```
(qemu) info registers
EAX=00000365 EBX=000000c4 ECX=00000000 EDX=00000080
CS =0020 00000000 ffffffff 00cf9800 DPL=0 CS32 [---]
DS =0008 00000000 ffffffff 00cf9300 DPL=0 DS   [-WA]
SS =0010 00000000 00000000 00409700 DPL=0 DS   [EWA]
GS =0018 000b8000 00007fff 00409300 DPL=0 DS   [-WA]
GDT=     00008800 00000027
CR0=00000011
```

从中可以清楚地看到段选择子已经生效（CS=0x0020, DS=0x0008），A20已打开，CR0的PE位已设置。这对于验证保护模式的4个步骤非常有用。
...
11. 复现“进入保护模式”一节，使用gdb或其他debug工具在进入保护模式的4个重要步骤上设置断点，并结合代码、寄存器的内容等来分析这4个步骤，最后附上结果截图。gdb的使用可以参考appendix的“debug with gdb and qemu”部份。
    ...
    > **ARM64 用户提示：** 如果你的 GDB 不支持 x86 调试，可使用 QEMU Monitor 的 `info registers` 命令来查看保护模式切换前后的寄存器变化。

12. 在进入保护模式后，编写并执行一个自己的32位汇编程序。

# 在麒麟操作系统上完成实验（验证记录）

> 以下记录了在麒麟操作系统（Kylin Linux Advanced Server V11，ARM64架构）上完成本实验的验证过程。
...
## Example 1 验证：加载 bootloader

在麒麟OS ARM64 上编译运行 Example 1，步骤与 Ubuntu 相同，仅将 `qemu-system-i386` 替换为 `qemu-system-x86_64`：
...
# 运行（使用 qemu-system-x86_64 代替 qemu-system-i386）
qemu-system-x86_64 -hda hd.img -serial null -parallel stdio
```

MBR 成功加载 bootloader 并显示 "run bootloader"。

## Example 2 验证：进入保护模式

同样在麒麟OS ARM64 上编译运行 Example 2：
...
程序成功进入保护模式并显示 "enter protect mode"。

## 符号表生成（ARM64）

ARM64 上使用 i686-elf 交叉工具链生成符号表：

```shell
nasm -g -f elf32 mbr.asm -o mbr.o
i686-elf-ld -o mbr.symbol -melf_i386 -N mbr.o -Ttext 0x7c00

# bootloader 需要先删除 org 伪指令
sed '/^org/d' bootloader.asm > bootloader_nosym.asm
nasm -g -f elf32 bootloader_nosym.asm -o bootloader.o
i686-elf-ld -o bootloader.symbol -melf_i386 -N bootloader.o -Ttext 0x7e00
```

## 使用 QEMU Monitor 调试保护模式

由于 ARM64 上的 GDB 仅支持 AArch64 架构，使用 QEMU Monitor 作为替代调试方案：
...
在 QEMU Monitor 中执行 `info registers`，可以验证保护模式已正确进入：

```
CS =0020 00000000 ffffffff 00cf9800 DPL=0 CS32 [---]    # 代码段选择子 0x20
DS =0008 00000000 ffffffff 00cf9300 DPL=0 DS   [-WA]    # 数据段选择子 0x08
SS =0010 00000000 00000000 00409700 DPL=0 DS   [EWA]    # 栈段选择子 0x10
GS =0018 000b8000 00007fff 00409300 DPL=0 DS   [-WA]    # 视频段选择子 0x18
GDT=     00008800 00000027                                # GDT在0x8800，界限39
CR0=00000011                                              # PE位已设置，保护模式已开启
```